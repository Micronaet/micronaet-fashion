...
def write_subtotal(Excel, page, row, old, f_total, f_empty):
    ...
    Excel.merge_cell(page, [row, 5, row, 11])
...
Excel.create_worksheet(page)
Excel.column_width(page, [block1_width[-1]] + block1_width[:-1])
...
header_block_2 = [block1_header[-1]] + block1_header[:-1]
